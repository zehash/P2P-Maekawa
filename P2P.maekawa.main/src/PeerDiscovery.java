...
public class PeerDiscovery {
    ...
    static ArrayList<Task> Threads = new ArrayList<Task>(10); // ArrayList to manage
                                                            // Node
                                                            // multi-threading
    public static ArrayList<PeerDiscoveryPacket> nodeTable = new ArrayList<PeerDiscoveryPacket>();
    ...
    class Task implements Runnable {
        ...
        public void run() {
            ...
            try { // read message and display it
                ...
                //removeData(this.connection.getInetAddress().getHostAddress().toString());
            } finally {
                
                closeConnection();
                removeData(this.connection.getInetAddress().getHostAddress());
                Threads.remove(this);
                reqcount++;
                activeNodes--;
                System.out
        ...
        public void prepareToSendRemovalDataToPeers(PeerDiscoveryPacket peerDiscoveryPacket){
            System.out.println("in prepare to send removal data, is removed value : "+peerDiscoveryPacket.getIsRemoved());
            for (int i = 0; i < Threads.size(); i++) {
            ...
            try { 
                ...
                System.out
                        .println("PEERDISCOVERY>>> Transmitting DATA PACKETS to PEERS "
                                ...
                                peerDiscoveryPacket.getServerStatus() + " AND ClientStat " + peerDiscoveryPacket.getClientStatus() + "isRemoved : "+peerDiscoveryPacket.getIsRemoved());
        ...
        private void removeData(String closingConnectionIP) {
            System.out.println("in Here!, closing connection "+closingConnectionIP);
            for(int i = 0; i < nodeTable.size(); i++) {
                System.out.println("Iterate : "+nodeTable.get(i).getIP());
                if(closingConnectionIP.compareTo(nodeTable.get(i).getIP()) == 0){
                    PeerDiscoveryPacket packetToSend = nodeTable.get(i);
                    packetToSend.setIsRemoved(true);
                    prepareToSendRemovalDataToPeers(packetToSend);
                    nodeTable.remove(i);
                }
            }
